...
```bash
grype hbactl:scan --fail-on high,critical
```

CI does the same: builds the image and runs `grype hbactl:scan --fail-on high,critical` (see `.github/workflows/security.yml`).
...
- **Security** workflow: runs on push/PR to `main` and `develop`. Jobs: **lint** (gofmt + gocyclo), **govulncheck** (Go deps), **Grype** (builds image, scans it with `--fail-on high,critical`).
