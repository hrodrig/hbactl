...
**Recommended before merge/release:** Run **`make check`** (go mod verify, build, test, lint, scan.sh). For a complete, release-grade scan of what you ship, also run **`make docker-scan`** (build image + Grype on it; see below).
...
This runs **go mod verify**, **go build ./...**, **go test ./...**, **make lint**, and **./tools/scan.sh** (govulncheck + optional Grype on dir). Exit code is non-zero if any step fails (e.g. govulncheck finds vulnerabilities), so you can gate merges on it.

To run only the security scan: `./tools/scan.sh`. Install govulncheck if needed: `go install golang.org/x/vuln/cmd/govulncheck@latest`. If **Grype** is on `PATH`, scan.sh also runs it against the repo (directory scan).
...
```bash
# From repo root (build + scan with --fail-on high)
make docker-scan
```
...
```bash
docker build -t hbactl:scan .
grype hbactl:scan
grype hbactl:scan --fail-on high   # fail on high and critical for scripting
```

CI does the same: builds the image and runs `grype hbactl:scan --fail-on high` (see `.github/workflows/security.yml`).

**What you'll see in practice:** A scan of the image often reports vulnerabilities in the **base image and OS packages** (Alpine: e.g. busybox, zlib, libc). Grype prints a table: package name, installed version, CVE ID, severity (Critical/High/Medium/Low), and sometimes EPSS. It's normal to have some **Medium** (or Low) findings in base layers. CI uses `--fail-on high` (fails on high and critical) so the job only fails on High/Critical; Medium findings are still visible in the log so you can track them. To reduce findings over time: pin a **newer Alpine base** in the Dockerfile (e.g. `FROM golang:1.26-alpine` or a newer `alpine:3.x` when the Go image allows it), rebuild and re-scan. Let the base image maintainers ship fixed versions; **do not** try to remove or upgrade individual base packages (e.g. zlib) in Alpine — that often breaks the image. Accept or document any remaining medium/low risk if they are not exploitable in our use case.

## CI

- **Security** workflow: runs on push/PR to `main` and `develop`. Jobs: **lint** (gofmt + gocyclo), **govulncheck** (Go deps), **Grype** (builds image, scans it with `--fail-on high`).
